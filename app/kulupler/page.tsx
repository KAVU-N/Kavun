      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="bg-white/85 backdrop-blur rounded-3xl shadow-xl border border-[#FFE5D9] px-10 py-12 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input value={q} onChange={e=>setQ(e.target.value)} className="px-4 py-3 rounded-xl border border-white/50 bg-white/70 backdrop-blur-sm placeholder:text-black/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FF8B5E]/60" placeholder={t('clubs.searchPlaceholder')} />
            <select value={category} onChange={e=>setCategory(e.target.value)} className="px-4 py-3 rounded-xl border border-white/50 bg-white/70 backdrop-blur-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FF8B5E]/60">
              <option value="">{t('clubs.categoryAll')}</option>
              {categories.map((c)=>(<option key={c.value} value={c.value}>{t(c.labelKey)}</option>))}
            </select>
            <select value={university} onChange={e=>setUniversity(e.target.value)} className="px-4 py-3 rounded-xl border border-white/50 bg-white/70 backdrop-blur-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FF8B5E]/60">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[1,2,3,4,5,6].map((i) => (
                <div key={i} className="rounded-2xl bg-white/70 backdrop-blur-sm border border-white/50 p-4 shadow-sm animate-pulse">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {clubs.length === 0 ? (
                ...
                <div key={club._id} className="relative rounded-2xl bg-white/75 backdrop-blur-md border border-white/60 p-5 pb-16 shadow hover:shadow-lg transition flex items-start gap-3">
                  <div className="w-12 h-12 rounded-md bg-white/80 border border-white/60 overflow-hidden flex-shrink-0 flex items-center justify-center text-sm text-[#994D1C] font-semibold">
                    {club.logoUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/kulupler/${club._id}`} className="block space-y-1">
                      <div className="font-semibold text-[#994D1C] line-clamp-1">{club.name}</div>
                      <div className="text-sm text-[#6B3416]/80 line-clamp-1">{club.university}{club.category ? ` • ${club.category}` : ''}</div>
                    </Link>
