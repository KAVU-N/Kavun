...
if (dd >= 0) {
  const profHref = s.indexOf('href="/profil"', dd);
  if (profHref >= 0) {
    const profStart = s.lastIndexOf('<Link', profHref);
    const kulHref = s.indexOf('href="/kuluplerim"', profStart);
    if (kulHref >= 0) {
      const kulStart = s.lastIndexOf('<Link', kulHref);
      const kulEnd = findClosing(s, kulStart, 'Link');
      if (profStart >= 0 && kulStart >= 0 && kulEnd > kulStart) {
        const indent = (s.slice(0, profStart).match(/\n(\s*)$/) || [,'                        '])[1];
        const block = [
          `${indent}<Link`,
          `${indent}  href="/profil"`,
        ...
        s = s.slice(0, profStart) + block + s.slice(kulEnd);
      }
    }
  }
}

// Remove any lone attribute lines that may remain
s = s.replace(/\n\s*href=\"\/projelerim\"\n/g, '\n');

fs.writeFileSync(target, s.replace(/\n/g, '\r\n'), 'utf8');
console.log('repairNavbarProfileLinksExact: completed');
